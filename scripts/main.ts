...
//footer year
const yearSpan: HTMLSpanElement | null = document.getElementById("footer-year");
if (yearSpan !== null) {
...
const msg: HTMLSpanElement | null = document.getElementById("msg");

if (form !== null) {
  form.addEventListener("submit", (e) => {
    e.preventDefault();
    const nameField = form["name"] as unknown as HTMLInputElement;
    const emailField = form["email"] as unknown as HTMLInputElement;
    const messageField = form["Message"] as unknown as HTMLTextAreaElement;
    ...
    fetch(scriptURL, { method: "POST", body: new FormData(form) })
      .then((response) => {
        if (msg !== null) msg.innerHTML = "Message sent successfully";
...
const closeBtn: HTMLElement | null = document.getElementById("close");
const openBtn: HTMLElement | null = document.getElementById("open");
const menu: HTMLElement | null = document.getElementById("small-screens");

if (openBtn !== null) openBtn.addEventListener("click", openMenu);
if (closeBtn !== null) closeBtn.addEventListener("click", closeMenu);

function openMenu() {
  if (menu !== null) menu.style.right = "0";
}

function closeMenu() {
  if (menu !== null) menu.style.right = "-200px";
}
...
const loadLastTwoBlogs = () => {
  ...
  const blogContainer =
    document.querySelector<HTMLDivElement>(".blog-container");
  ...
  if (blogs.length === 0) {
    ...
    if (blogContainer !== null) blogContainer.appendChild(noBlogs);
  } else {
    ...
    for (let i = 0; i < lastTwoBlogs.length; i++) {
      ...
      if (blogContainer !== null) blogContainer.appendChild(blogCard);
    }
  }
};
loadLastTwoBlogs();
