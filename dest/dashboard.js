...
const closeAddBlogModel = document.querySelector('[data-close-id="close-add-blog-modal"]');
    ...
    if (blogs.length === 0) {
        ...
        >
          ...
          <button type="button" onClick={editBlog(${blog.id})}>edit blog</button>
            ...
            const openEditBlogButton = document.getElementById(`open-edit-blog-model-${blog.id}`);
            const closeEditBlogButton = editModal.querySelector(`[data-close-id="close-edit-blog-modal-${blog.id}"]`);
          ...
          <div class="delete-modal">
            ...
            const openDeleteBlogModel = document.getElementById(`open-delete-blog-model-${blog.id}`);
            const closeDeleteBlogModel = document.getElementById(`close-delete-blog-modal-${blog.id}`);
            const noDeleteBlogModel = document.getElementById(`no-delete-blog-modal-${blog.id}`);
          ...
          <div class="comments-modal-header">
            ...
            const openCommentBlogModel = document.getElementById(`open-comments-blog-model-${blog.id}`);
            const closeCommentBlogModel = document.querySelector(`[data-close-id="close-comments-blog-modal-${blog.id}"]`);
        ...
        setTimeout(() => {
            ...
            <div class="delete-modal">
              ...
              <div class="delete-modal-buttons">
                ...
                const continueButton = document.getElementById(`continue-delete-blog-comment-${allComments[i].id}`);
                if (continueButton !== null) {
                    continueButton.addEventListener("click", () => deleteBlogComment(parseInt(allComments[i].blogId, 10), allComments[i].id));
                }
                //open and close comment delete blog model
                const openDeleteBlogCommentModel = document.getElementById(`open-delete-blog-comment-model-${allComments[i].id}`);
                if (openDeleteBlogCommentModel !== null) {
                    openDeleteBlogCommentModel.addEventListener("click", () => {
                        const currentOpenCommentModal = document.getElementById(`comments-blog-modal-${allComments[i].blogId}`);
                ...
                const closeDeleteBlogCommentModel = document.querySelector(`[data-close-id="close-delete-blog-comment-modal-${allComments[i].id}"]`);
                const noDeleteBlogCommentModel = document.querySelector(`[data-close-id="no-delete-blog-comment-modal-${allComments[i].id}"]`);
...
const saveNewBlog = () => {
    // get blogs from local storage
    let blogs = JSON.parse(localStorage.getItem("blogs") || "[]");
    const addBlogform = document.forms.namedItem("add-blog-form");
    if (addBlogform !== null) {
        const titleField = addBlogform["title"];
        const contentField = addBlogform["content"];
        const poster = addBlogform["poster"].files.length > 0
            ? addBlogform["poster"].files[0].name
            : "default.jpg";
...
const editBlog = (blogId) => {
    let blogs = JSON.parse(localStorage.getItem("blogs") || "[]");
    const editBlogform = document.forms.namedItem(`edit-blog-form-${blogId}`);
    if (editBlogform !== null) {
        const titleField = editBlogform["title"];
        const contentField = editBlogform["content"];
        const poster = editBlogform["poster"].files.length > 0
            ? editBlogform["poster"].files[0].name
            : "default.jpg";
        if (titleField.value.trim() === "") {
            alert("Please enter a title.");
            return;
    ...
    const currentOpenedModal = document.getElementById(`edit-blog-modal-${blogId}`);
...
const deleteBlog = (blogId) => {
    ...
    localStorage.setItem("blogs", JSON.stringify(blogs));
    //current opened modal
    const currentOpenedModal = document.getElementById(`delete-blog-modal-${blogId}`);
    if (currentOpenedModal !== null)
        currentOpenedModal.style.display = "none";
    const blogMessage = document.getElementById("blog-message");
    if (blogMessage !== null) {
...
const deleteBlogComment = (blogId, commentId) => {
    ...
    const commentIndex = blogs[blogIndex].comments.findIndex((comment) => comment.id === commentId);
    if (commentIndex === -1) {
    ...
    localStorage.setItem("blogs", JSON.stringify(blogs));
    //current opened modal
    const currentOpenedModal = document.getElementById(`delete-blog-comment-modal-${commentId}`);
    if (currentOpenedModal !== null)
        currentOpenedModal.style.display = "none";
    const blogMessage = document.getElementById("blog-message");
    if (blogMessage !== null) {
...
const setLikeCommentsandViewNumbers = () => {
    ...
    if (comments !== null)
        comments.innerHTML = allComments.length.toString();
    let likesNumber = 0;
    for (let i = 0; i < blogs.length; i++) {
