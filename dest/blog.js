...
const loadBlog = (currentBlog) => {
    ...
    if (views !== null)
        views.innerHTML = currentBlog.views.toString();
...
const updateViews = (currentBlog, blogId) => {
...
const closeModal = document.querySelector('[data-close-id="close-comments-modal"]');
if (closeModal !== null) {
...
const addcomment = () => {
    if (blogIdParam !== null) {
        ...
        let blogs = JSON.parse(localStorage.getItem("blogs") || "[]");
        const currentBlog = blogs.find((blog) => blog.id === blogId);
        const form = document.forms.namedItem("add-comment-to-blog");
        //get form values
        const contentField = form["comment"];
